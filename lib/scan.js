  ...
  Poke.init(opts.username, opts.password, opts.location, opts.provider, (err) => {
    ...
    const interval = setInterval(() => {
      Poke.Heartbeat((err, hb) => {
        if (err) {
          console.log(err)
          return module.exports(opts, targetedPokemon, notifications, cb);
          // throw err;
        }
        ...
        .map(cell => {
          ...
          if (targets[cell.name]) {
            console.log(`Already found a ${cell.name}, skipping...`);
            return;
          }
          ...
          return Map.generate(opts.location, cell, (map) => {
            ...
            if (notifications.text) {
              ...
              Text.sendMessage(cell, notifications.text)
            }
            ...
            console.log(`\nFOUND A ${cell.name} | ${_.filter(targets, t => t).length}/${_.keys(targets).length}\n`);
            return;
          });
        })
        .value();
